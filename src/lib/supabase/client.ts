...
export function createClient(): SupabaseClient {
  ...
  const client = createBrowserClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      ...
      // NB: we deliberately DO NOT pass a custom `cookies` option here.
      // Slimming the auth cookie is done **server-side only** (in
      // `middleware.ts` and `lib/supabase/server.ts`) — overriding
      // document.cookie I/O in the browser broke supabase-js' internal
      // refresh path (the standard `Cannot read properties of undefined`
      // error after a tab regained focus). On the server the slim happens
      // right before `Set-Cookie` is written, so the browser receives a
      // small cookie and replays it on the next request — exactly what
      // we need to avoid the oversized-cookie 502.
    },
  );
